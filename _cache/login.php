...
<body>
	<p id="backtoblog">
		<a href="../">&larr; 返回微博首页</a>
	</p>
	<div id="login">
		<h1>果果微博管理后台</h1><?php
		if ($_obj ['loginResult'] == "error") {
			?><div id="login_result">
			<strong>错误</strong>：用户名与密码不匹配。
		</div><?php
		}
		?><?php
		...
		?><form name="loginform" id="loginform" action="./login.php"
			method="post">
			<p>
				<label>用户名<br /> <input type="text" name="username" id="username"
					class="input" value="" size="20" tabindex="10" /></label>
			</p>
			<p>
				<label>密码<br /> <input type="password" name="password" id="password"
